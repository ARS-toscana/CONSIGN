...
# 
# #Broad
# MFC_MAJORCA_broad <- MergeFilterAndCollapse(list(MAJORCA_possible, MAJORCA_narrow),
#                                              datasetS= D3_gop_for_outcomes,
#                                              key = "person_id",
#                                              condition = "date >= pregnancy_start_date & date <= pregnancy_end_date",
#                                              strata=c("pers_group_id"),
#                                              summarystat = list(list(c("exist"), "date" , "MAJORCA_broad")))
# 
# pregnancy_outcomes_6 <- merge(pregnancy_outcomes_5, MFC_MAJORCA_broad, by = c("pers_group_id"), all.x = T)[is.na(MAJORCA_broad), MAJORCA_broad := 0]
pregnancy_outcomes_6 <- pregnancy_outcomes_4
...
# 
# #Broad
# MFC_MATERNALDEATH_broad <- MergeFilterAndCollapse(list(MATERNALDEATH_possible, MATERNALDEATH_narrow),
#                                                    datasetS= D3_gop_for_outcomes,
#                                                    key = "person_id",
#                                                    condition = "date >= pregnancy_start_date & date <= pregnancy_end_date",
#                                                    strata=c("pers_group_id"),
#                                                    summarystat = list(list(c("exist"), "date" , "MATERNALDEATH_broad")))
# 
# pregnancy_outcomes_8 <- merge(pregnancy_outcomes_7, MFC_MATERNALDEATH_broad, by = c("pers_group_id"), all.x = T)[is.na(MATERNALDEATH_broad), MATERNALDEATH_broad := 0]


PERSONS <- fread(paste0(dirinput,"PERSONS.csv"))
pregnancy_outcomes_7 <- merge(pregnancy_outcomes_6, PERSONS[,.(person_id, day_of_death, month_of_death, year_of_death)], by = "person_id", all.x = T)
...
# 
# #Broad
# MFC_MICROCEPHALY_broad <- MergeFilterAndCollapse(list(MICROCEPHALY_possible, MICROCEPHALY_narrow),
#                                                   datasetS= D3_gop_for_outcomes,
#                                                   key = "person_id",
#                                                   condition = "date >= pregnancy_start_date & date <= pregnancy_end_date",
#                                                   strata=c("pers_group_id"),
#                                                   summarystat = list(list(c("exist"), "date" , "MICROCEPHALY_broad")))
# 
# pregnancy_outcomes_10 <- merge(pregnancy_outcomes_9, MFC_MICROCEPHALY_broad, by = c("pers_group_id"), all.x = T)[is.na(MICROCEPHALY_broad), MICROCEPHALY_broad := 0]
pregnancy_outcomes_10 <- pregnancy_outcomes_8
################################################################################
#########################     Pre eclampsia      ###############################
################################################################################
#Narrow
...
# 
# #Broad
# MFC_PRETERMBIRTH_broad <- MergeFilterAndCollapse(list(PRETERMBIRTH_possible, PRETERMBIRTH_narrow),
#                                                   datasetS= D3_gop_for_outcomes,
#                                                   key = "person_id",
#                                                   condition = "date >= pregnancy_start_date & date <= pregnancy_end_date",
#                                                   strata=c("pers_group_id"),
#                                                   summarystat = list(list(c("exist"), "date" , "PRETERMBIRTH_broad")))
# 
# pregnancy_outcomes_14 <- merge(pregnancy_outcomes_13, MFC_PRETERMBIRTH_broad, by = c("pers_group_id"), all.x = T)[is.na(PRETERMBIRTH_broad), PRETERMBIRTH_broad := 0]
...
################################################################################
# denominator
D3_pregnancy_outcomes <- pregnancy_outcomes_21[highest_quality == "B_Yellow" | highest_quality == "A_Green", 
...
# ageband
#PERSONS <- fread(paste0(dirinput,"PERSONS.csv"))
D3_pregnancy_outcomes <- merge(D3_pregnancy_outcomes, PERSONS[,.(person_id, day_of_birth, month_of_birth, year_of_birth)], by = "person_id", all.x = T)
...
rm(list = c( "MFC_INSULIN",
             ...
             #"MFC_MAJORCA_narrow",
             #"MFC_MAJORCA_broad",
             #"MFC_MICROCEPHALY_narrow",
             #"MFC_MICROCEPHALY_broad",
...
rm(INSULIN_first_record, D3_pregnancy_outcomes, PERSONS,
