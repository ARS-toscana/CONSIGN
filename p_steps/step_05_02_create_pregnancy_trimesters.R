...
D3_preg_trim_2 <- merge(D3_preg_trim_2, D3_PERSONS[,.(person_id, date_of_birth)], 
                                all.x = T, by = "person_id")

D3_preg_trim_2 <- D3_preg_trim_2[, age:= as.integer((pregnancy_start_date - date_of_birth) / 365)]
D3_preg_trim_2 <- D3_preg_trim_2[, -c("date_of_birth")]

D3_preg_trim_2 <- D3_preg_trim_2[age>=12 & age<=34, age_category := 1]
D3_preg_trim_2 <- D3_preg_trim_2[age>=35 & age<=55, age_category := 2]
...
D3_pregnancy_risk_trimester <- D3_pregnancy_risk_trimester[is.na(status), status := 0]
...
rm(D3_pregnancy_risk_trimester,D3_preg_trim_2, D3_preg_trim,
   MFC_covid_in_trim, covid_registry, SURVEY_ID_COVID)